...
class AuditLogger:
    ...
    def start_run(
        ...
        if not job_run_id:
            raise ValueError(
                "job_run_id must be provided in job_context and not be empty"
            )
    ...
    def complete_run(
        ...
        throughput_mb_per_sec: float | None = None,
        copy_duration_sec: float | None = None,
        ...
        if status not in (AUDIT_STATUS_SUCCESS, AUDIT_STATUS_FAILED):
            raise ValueError(
                f"Final audit status must be SUCCESS or FAILED, got {status!r}"
            )
    ...
    def fail_run(
        self, audit_run: dict[str, Any], error_code: str, error_message: str
    ) -> None:
    ...
    def log_skipped_row(
        ...
        row = [
            (
                ...
                0,
                0,
                0,  # rows_* and byte metrics
                None,
                None,  # throughput, copy_duration
                self._required_string(ctx.get("notebook_name"), "UNKNOWN"),
                self._required_string(ctx.get("databricks_url"), "UNKNOWN"),
                ...
                reason
                or "Source lookup returned 0 rows — table excluded from ingestion.",
            )
        ]
